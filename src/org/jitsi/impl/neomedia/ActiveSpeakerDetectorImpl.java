 ...
 * {@inheritDoc}
 *
 * For each stream, it keeps the last {@link org.jitsi.impl.neomedia.ActiveSpeakerDetectorImpl.History#SIZE}
 * levels and uses them to compute a score.
 *
 * At all times one stream (indicated in {@link #active}) is considered active,
 * while the rest are considered 'competing'.
 *
 * When a new audio level is received for a stream, its score is recomputed,
 ...
 * 1. Helps to avoid often changing the active when there are two streams
 *      with similar levels.
 * 2. This is to prevent switching the active stream away during times of
 *      "silence". Without this threshold we observed the following: someones
 *      microphone generates noise with levels above the levels of the active
 *      speaker. When the active speaker pauses speaking, the one with the higher
 *      noise becomes active.
 * 3. This is for the case when someone quits the conference shouting ;)
 * 4. This is because of the way we compute scores. Just-added streams might have
 *      an uncharacteristically high score.
 *
 *
 *
 * @author Boris Grozev
    ...
    extends ActiveSpeakerDetector
{
    ...
    static String A = "grepmeout";

    private final Map<Long, History>histories
            = new HashMap<Long, History>();

    private final ReadWriteLock historiesLock
    ...
    private class History
    {
        ...
        private synchronized void update(int level)
        {
            ...
            //if(score>110)
            //        System.err.println(""+A+" "+ssrc+" update("+level+") score="+score);
        }
