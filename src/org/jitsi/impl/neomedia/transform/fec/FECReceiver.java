    ...
    public synchronized RawPacket[] reverseTransform(RawPacket[] pkts)
    {
        ...
        for (int i = 0; i < pkts.length; i++)
        {
            ...
            if (pkt.getPayloadType() == ulpfecPT)
            {
                ...
                // (hence the same seq., hence no need to "consume" a seq.)
                nbFec++;
            ...
            else // media, save it _before_ changing seq.
            {
                if (handleFec)
                    saveMedia(pkt);
                ...
                if (nbFec != 0)
                {
                    // perform the seq. change operation (e.g. cut its number
                    // by nbFec)
                    pkt.setSequenceNumber(
                            (pkt.getSequenceNumber() - nbFec + (1<<16)));
                }
            }
        }
            ...
            for (Map.Entry<Integer, RawPacket> entry : fecPackets.entrySet())
            {
                ...
                if (reconstructor.canRecover())
                {
                    ...
                    if (recovered != null)
                    {
                        nbRecovered++;
                        saveMedia(recovered);
                        // +1 because the fec packet used for recovery has
                        // already been counted in nbFec, but comes *after*
                        // the recovered packet.
                        recovered.setSequenceNumber(
                                recovered.getSequenceNumber() - nbFec + 1);
