    ...
    implements VideoMediaStream
{
    ...
    /**
     * The <tt>TransformEngine</tt> used to remove RED encapsulation from
     * incoming packets and/or add RED encapsulation to outgoing packets.
     */
    private REDTransformEngine redTransformEngine;

    /**
     * The <tt>TransformEngine</tt> used used to handle incoming packets with
     * payload type ulpfec, and/or to generate outgoing ulpfec packets
     */
...
//                        USE_PLI = true;
    ...
    protected FECTransformEngine getFecTransformEngine()
    {
        if (fecTransformEngine == null)
            fecTransformEngine = new FECTransformEngine();
    ...
    protected REDTransformEngine getRedTransformEngine()
    {
        if (redTransformEngine == null)
            redTransformEngine = new REDTransformEngine();
        return redTransformEngine;
    }
    ...
    public void close()
    {
        if (redTransformEngine != null)
        {
            redTransformEngine.close();
            redTransformEngine = null;
        }

        if (fecTransformEngine != null)
        {
            fecTransformEngine.close();
            fecTransformEngine = null;
        }
