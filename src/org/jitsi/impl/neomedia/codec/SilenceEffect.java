    ...
    implements Effect
{
     ...
     * TODO: refactor to allow different sampling rates.
     */
    private static final int sampleRate = 48000;

    /**
     * The size of a single sample of input in bits.
     * TODO: refactor to allow different sizes
     */
     ...
     */
    private static final int MAX_SAMPLES_SILENCE = sampleRate * 10;
    ...
    protected int doProcess(Buffer inBuf, Buffer outBuf)
    {
        ...
        else // timestamp != -1 && lastOutputTimestamp != -1
        {
            ...
            {
                logger.warn("Calculated that " + diffSamples + " samples of "
                            + "silence need to be inserted. Only inserting the"
                            + " configured maximum of " + MAX_SAMPLES_SILENCE);

                lastOutputTimestamp = timestamp - MAX_SAMPLES_SILENCE;
                diffSamples = MAX_SAMPLES_SILENCE;
            }
    ...
    private long calculateTimestamp(long oldTimestamp, long samplesToAdd)
    {
        ...
        //XXX: need to handle wrap of the Buffer.getTimestamp() timestamps?
